...
import { FileIcon, SparkleIcon } from "@/components/ui/icons";

// ─── Extracted chat sub-components ───
import { AutomationBanner, useChatSession, MessageItem, ToolItem } from "@/components/chat";
    ...
    <div className="flex h-full w-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        {activeConversationAutomatic && activeConversation && (
          <AutomationBanner conversation={activeConversation} messages={messages} />
        )}
        ...
        >
          {!activeConversationId ? (
            ...
            <div className="flex flex-col gap-3 max-w-4xl lg:max-w-5xl mx-auto pt-20 md:pt-28">
              {(() => {
                type TimelineItem =
                ...
                const timeline: TimelineItem[] = [
                  ...
                  const rank = (entry: TimelineItem) => {
                    if (entry.kind === "tool") return 1;
                    if (entry.item.role === "user") return 0;
                    return 2; // assistant
                  };
              ...
              {sending && !streamingMsgId && (
                <div className="flex items-start gap-3 animate-in fade-in duration-200">
                  <div className="hidden sm:flex shrink-0 mt-1 h-11 w-11 rounded-full bg-[#0d0d1a] border border-white/10 items-center justify-center shadow-[0_0_12px_rgba(0,0,0,0.5)]">
                    <SparkleIcon className="text-violet-light w-5 h-5 animate-pulse" />
                  </div>
                  <div className="bg-surface-high border border-white/8 rounded-2xl px-5 py-4 flex items-center gap-1.5">
                    <span
