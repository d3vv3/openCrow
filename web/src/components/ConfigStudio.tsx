...
function SSHServerCard({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      updateConfig((c) => { c.integrations.sshServers[i].sshKey = reader.result as string; return c; });
    };
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-surface-mid overflow-hidden">
      ...
      >
        <AnimatedDot status={testing ? "pending" : testResult ? (testResult.ok ? "ok" : "error") : (server.enabled ? "ok" : "idle")} />
        <span className="font-medium text-sm flex-1 truncate">{server.name || `Server ${i + 1}`}</span>
        {server.host && <span className="text-xs text-on-surface-variant truncate hidden sm:block">{server.username}@{server.host}</span>}
        {testResult && (
          <span className={`flex items-center gap-1.5 text-xs font-mono px-2 py-0.5 rounded ${testResult.ok ? "text-cyan bg-cyan/10" : "text-error bg-error/10"}`}>
            <AnimatedDot status={testResult.ok ? "ok" : "error"} />
            {testResult.ok ? "OK" : "FAIL"}
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-white/10 pt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Server Name" value={server.name} placeholder="my-server" onChange={(e) => {
              const v = e.target.value.replace(/[^a-zA-Z0-9_-]/g, "");
              updateConfig((c) => { c.integrations.sshServers[i].name = v; return c; });
            }} />
          ...
          <div className="flex items-center gap-3">
            ...
            <Toggle label="" checked={server.authMode === "password"} onChange={(v) => updateConfig((c) => { c.integrations.sshServers[i].authMode = v ? "password" : "key"; return c; })} />
            <span className="text-sm text-on-surface-variant">Password</span>
          </div>

          {server.authMode === "password" ? (
            <Input label="Password" type="password" value={server.password ?? ""} onChange={(e) => updateConfig((c) => { c.integrations.sshServers[i].password = e.target.value; return c; })} />
          ) : (
            <div className="space-y-2">
              <TextArea
                label="Private Key"
                value={server.sshKey ?? ""}
                onChange={(e) => updateConfig((c) => { c.integrations.sshServers[i].sshKey = e.target.value; return c; })}
                rows={6}
              ...
              <div className="flex items-center gap-4">
                <label className="inline-flex items-center gap-2 cursor-pointer text-sm text-cyan hover:text-cyan/80">
                  <input type="file" className="hidden" accept=".pem,.key,.pub,*" onChange={handleFileUpload} />
                  Upload key file
                </label>
              </div>
              <Input label="Passphrase (optional)" type="password" value={server.passphrase ?? ""} onChange={(e) => updateConfig((c) => { c.integrations.sshServers[i].passphrase = e.target.value; return c; })} />
            </div>
          )}

          <div className="flex items-center gap-4">
            <Toggle label="Enabled" checked={server.enabled} onChange={(v) => updateConfig((c) => { c.integrations.sshServers[i].enabled = v; return c; })} />
          </div>
          ...
          <div className="flex gap-2 pt-2 items-center">
            ...
            {testResult && (
              <span className={`flex items-center gap-1.5 text-xs font-mono ${testResult.ok ? "text-cyan" : "text-error"}`}>
                <AnimatedDot status={testResult.ok ? "ok" : "error"} />
                {testResult.ok ? "Connected" : testResult.error}
              </span>
            )}
            <Button variant="ghost" size="sm" className="ml-auto hover:text-error" onClick={() => updateConfig((c) => { c.integrations.sshServers.splice(i, 1); return c; })}>
              Remove
            </Button>
          </div>
        </div>
...
function ServersTab({
  ...
  return (
    <div className="space-y-6">
        ...
        action={
          <Button variant="secondary" size="sm" onClick={() => updateConfig((c) => {
            c.integrations.sshServers.push({
              ...
              authMode: "key", sshKey: "", password: "", enabled: true,
            });
            return c;
          })}>
            Add Server
      ...
      <SaveBar onClick={saveFullConfig} loading={saving} label="Save Servers Config" status={saveStatus} />
    </div>
  );
}
