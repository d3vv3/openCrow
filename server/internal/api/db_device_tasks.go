...
func (s *Server) createDeviceTask(ctx context.Context, userID, targetDevice, instruction string) (DeviceTaskDTO, error) {
	var dto DeviceTaskDTO
	q := `
INSERT INTO device_tasks (user_id, target_device, instruction, status)
VALUES ($1, $2, $3, 'pending')
RETURNING id, target_device, instruction, status, result_output, created_at, updated_at, expires_at;
`
	var resultOutput *string
	var createdAt, updatedAt time.Time
	var expiresAt *time.Time

	err := s.db.QueryRow(ctx, q, userID, targetDevice, instruction).Scan(
		&dto.ID, &dto.TargetDevice, &dto.Instruction, &dto.Status,
...
func (s *Server) listDeviceTasks(ctx context.Context, userID string) ([]DeviceTaskDTO, error) {
	q := `
SELECT id, target_device, instruction, status, result_output, created_at, updated_at, expires_at
FROM device_tasks
WHERE user_id = $1
